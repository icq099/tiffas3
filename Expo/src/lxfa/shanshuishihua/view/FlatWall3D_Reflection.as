package lxfa.shanshuishihua.view{
    ...
    public class FlatWall3D_Reflection extends Sprite 	{
		...
		private function init3DObject():void {
			...
			for (var i:int = 0; i < itemOfNumber; i++) {				
				if (i % 2 == 0) {	
					...
					bmpMat = new ReflectionFileMaterial("img/shanshuishihua/images/" + i + ".jpg",true);
					//反射材質
					planeHeight = 400;
					//因為使用反射材質, 所以高度要增加一倍					
				}else {
					bmpMat = new BitmapFileMaterial("img/shanshuishihua/images/" + i + ".jpg");
					planeHeight = 240;
					//使用本來的點陣圖材質和高度
				}				
		...
		private function onEvent3DClick(e:InteractiveScene3DEvent):void{
            ...
            fp.myPlay("video/shanshuishihua/flvs/"+_target.getID()+".flv");
            PopUpManager.centerPopUp(fp);
            fp.x=this.parent.parent.parent.parent.parent.x;
            fp.y=this.parent.parent.parent.parent.parent.y;
